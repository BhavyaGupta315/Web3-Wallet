...
export default function WalletBar({ publicKeys, privateKeys, mode, index, setPrivateKeys, setPublicKeys }) {
    ...
    const deleteWallet=  (indexToDelete)=>{
        ...
        const newSetOfPublicKeys = [...publicKeys.slice(0, indexToDelete), ...publicKeys.slice(indexToDelete+1)];
        const newSetOfPrivateKeys = [...privateKeys.slice(0, indexToDelete), ...privateKeys.slice(indexToDelete+1)];
        setPublicKeys([newSetOfPublicKeys]);
        setPrivateKeys([newSetOfPrivateKeys]);
    }

    return (
        <div className="h-[18rem] rounded-lg p-2 bg-[#303030]">
            ...
            <div className={`${mode}-wallet-css`}>
                <div className="my-2">
                    ...
                    <p className="break-words text-sm my-1 cursor-pointer text-glow-hover" onClick={()=>handleCopy(publicKeys[index])}>{publicKeys[index]}</p>
                </div>
                <div className="my-2">
                    <p className="text-2xl font-extrabold my-1">Private Key</p>
                    <p className="break-words text-sm my-1 cursor-pointer" onClick={()=>handleCopy(privateKeys[index])}>{seen ? privateKeys[index] : dummyPrivateKey}</p>
                </div>
